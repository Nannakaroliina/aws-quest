/* =========================================================================
   ...
   - Music: a looping <audio id="bgm"> chiptune track (static/assets/bgm.mp3),
   ...
   Ported from the pre-SvelteKit src/audio.js: same behaviour, now an ES module
   exporting the `sound` singleton instead of window.AWSQUEST_SOUND. Touches no
   DOM/Audio API at module load, so it is safe to import anywhere.
   ========================================================================= */

type WaveType = OscillatorType;
type Note = [freq: number, dur: number, type?: WaveType, vol?: number];

interface BgmAudio extends HTMLAudioElement {
  _fade?: ReturnType<typeof setInterval>;
}
...
function createSound() {
  ...
  function seq(notes: Note[], type: WaveType = 'square', gap = 0) {
    ensure();
    if (!ctx || muted) return;
  ...
  function noise(dur = 0.12, vol = 0.6) {
    ...
    g.gain.value = vol;
    srcNode.buffer = buf;
    srcNode.connect(g).connect(master);
    srcNode.start();
  }
  ...
  return {
    ...
    type() {
      if (Math.random() < 0.5) seq([[N(10 + ((Math.random() * 4) | 0)), 0.012, 'square', 0.5]]);
    },
    ...
    wrong() {
      seq(
        [
          [N(-1), 0.1, 'sawtooth', 0.9],
          [N(-4), 0.18, 'sawtooth', 0.9],
        ],
        'sawtooth',
      );
      noise(0.1, 0.22);
...
export const sound: Sound = createSound();
